...
Pengcheng Zhang is a senior engineer and researcher at **Tencent Big Data**. He works on **TEngine**, a general-purpose heterogeneous query execution engine. TEngine serves as the unified execution layer for **TEngineDB (MPP)** and **TEngine Spark (BSP)**, enabling efficient execution across CPUs, GPUs, and NPUs.
...
<div class="about-zh" lang="zh-CN">
  <p>张鹏程现任腾讯大数据高级工程师，从事通用异构查询执行引擎 TEngine 的研发。作为通用计算底座，TEngine 面向 CPU、GPU、NPU 等异构硬件，为 TEngineDB（MPP）和 TEngine Spark（BSP）提供统一、高效的执行能力。博士毕业于上海交通大学，师从姚斌教授。</p>
